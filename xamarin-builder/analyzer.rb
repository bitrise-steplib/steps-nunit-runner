...
class Analyzer
  ...
  def build_solution_command(config, platform)
    [
        MDTOOL_PATH,
        'build',
        "\"-c:#{mdtool_configuration("#{config}|#{platform}")}\"",
        @solution[:path]
    ].join(' ')
  end
  ...
  def mdtool_configuration(project_configuration)
    configuration = project_configuration.split('|').last

    if configuration.eql?("AnyCPU") || configuration.eql?("Any CPU")
      return project_configuration.split('|').first
    end
